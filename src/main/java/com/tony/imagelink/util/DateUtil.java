 ...
 * </p>
 * <p>
 * 该类负责对日期格式化转换、日期比较、日期加减、润年判断、获取相关的日期信息等。
 * </p>
 *
 */
public class DateUtil {
    ...
    public static String formatData = "yyyy-MM-ddHH:mm:ss";
     ...
     * @return
     */
    public static long getTimeDifferenceDay(Date startDate,Date endDate){
        long l = endDate.getTime()-startDate.getTime();       //获取时间差
    ...
    public static Date getDateAfterHours(Date start,int hours){
        return add(Calendar.HOUR_OF_DAY,hours,start);
    }
     ...
     * 计算N分钟后的日期
     * @param start 开始日期
     * @param minutes 可以为负，表示前N分钟
     * @return 新的日期
     */
    public static Date getDateAfterMinutes(Date start, int minutes){
        return add(Calendar.MINUTE, minutes, start);
    }
